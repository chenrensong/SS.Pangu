using SS.NLP.International.Chinese.Translate;
using SS.Pangu.Framework;
using System;
            ...
            while (cur != null)
            {
                ...
                switch (cur.Value.WordType)
                {
                    case WordType.SimplifiedChinese:
                        ...
                        {
                            string simplified = ChineseTranslate.Convert(cur.Value.Word, ChineseTranslateDirection.TraditionalToSimplified);
                        ...
                        while (curChsMatch != null)
                        {
                            ...
                            if (_Options.OutputSimplifiedTraditional)
                            {
                                if (_Options.TraditionalChineseEnabled)
                                {
                                    ...
                                    {
                                        newWord = ChineseTranslate.Convert(wi.Word, ChineseTranslateDirection.TraditionalToSimplified);
                                        wt = WordType.TraditionalChinese;
                                    }
                                    else
                                    {
                                        newWord = ChineseTranslate.Convert(wi.Word, ChineseTranslateDirection.SimplifiedToTraditional);
            ...
            if (_Options.SynonymOutput)
            {
                ...
                while (node != null)
                {
                    ...
                    if (synonyms != null)
                    {
                        ...
                        {
                            var sourceWord = node.Value.Word;
                            if (!(node.Value.WordType != WordType.Synonym))
                            {
                                sourceWord = node.Value.OriginalWord;
                            }
                            node = result.AddAfter(node, new WordInfo(word, node.Value.Position,
                                ...
                                WordType.Synonym, node.Value.WordType, sourceWord));
                        }
                    }
